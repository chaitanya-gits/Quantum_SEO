...
PAGERANK_SQL = """
SELECT target_url AS url, COUNT(*)::float AS score
FROM page_links
WHERE target_url = ANY($1::text[])
GROUP BY target_url
"""
...
class PostgresStorage:
    ...
    async def search_pages(self, query: str, limit: int = 10) -> list[SearchRecord]:
        rows = await self.pool.fetch(SEARCH_PAGES_SQL, query, limit)
    ...
    async def get_pagerank_scores(self, urls: list[str]) -> dict[str, float]:
        ...
        rows = await self.pool.fetch(PAGERANK_SQL, urls)
        return {str(row["url"]): float(row["score"]) for row in rows}
